...
| `actualDrinks` | `number \| null` | 実際に飲んだ杯数（0〜8。8は「8杯以上」を意味する） | `null` |
...
| 関数 | 説明 |
| :--- | :--- |
| `toLocalDateStr(d)` | ローカルタイムゾーンで`"YYYY-MM-DD"`を返す（UTCずれ防止） |
| `todayStr()` | 今日の日付を`"YYYY-MM-DD"`で返す（内部で`toLocalDateStr`使用） |
